...
import 'package:managerdesktop/models/manager_report.dart';
...
class ReservationProvider extends BaseProvider<Reservation> {
  ...
  Future<ManagerReport> getManagerReport() async {
    final uri = Uri.parse(baseUrl).resolve('/Reservation/report');
    final headers = createHeaders();
    ...
    if (response.statusCode == 200) {
      final Map<String, dynamic> decoded = jsonDecode(response.body);
      return ManagerReport.fromJson(decoded);
    } else {
      throw Exception("Failed to get manager report: ${response.statusCode}");
    }
  }
}